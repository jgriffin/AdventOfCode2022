...
extension Day12Tests {
    struct HeightMap {
        ...
        init(_ squares: [[Square]]) {
            ...
            allIndices = IndexXY.allIndexXY(squares.indexXYRanges())
            start = allIndices.first(where: { squares[$0] == .start })!
            goal = allIndices.first(where: { squares[$0] == .end })!
            ...
            reachableNeighbors = { (index: IndexXY) in
        ...
        func heightDifference(_ indexXY: IndexXY, from _: IndexXY) -> Int {
